...
def _get_version_summary():
    """Get version summary of ``prettyplots``.
    ...
    -------
    summary : `str`
        Version summary of ``prettyplots``.
    """
    # Check versions of prettyplots and numpy.
    from . import _version
    if _version.version != version:
        raise ValueError("prettyplots version has changed since "
                         "installation/compilation")



    # Generate summary.
    summary = ("prettyplots {prettyplots_ver!s};\n"
               "git revision {git_rev!s} using\n"
               "python {python_ver!s}\n")
    
    summary = summary.format(prettyplots_ver=full_version,
